...
bool recursivePermute(struct personInfo person[], int k, int numOfPeople, int disallowedPairCount, char disallowedPairs[disallowedPairCount][2][19]){
    int j;
    ...
    if (k == numOfPeople){	
		...
		// If all arrangements are valid, print them and return true
        ...
        for (j = k; j < numOfPeople; ++j){
				...
				if (recursivePermute(person, k + 1, numOfPeople, disallowedPairCount, disallowedPairs)){
					return true;
				}
                swap(person, j, k);
        }
    }	
	// If no valid arrangement found, return false
	return false;
}
...
int main(int argc, char *argv[]) {
	...
	// Generate permutations and print result in alphabetical order
	recursivePermute(people, 0, numOfPeople, disallowedPairCount, disallowedPairs);
	return 0;
}
